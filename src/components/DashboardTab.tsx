...
export default function DashboardTab({
  ...
  useEffect(() => {
    const status = currentGlucose < currentGoal.low ? 'low' : currentGlucose > currentGoal.high ? 'high' : 'normal';
    ...
    if (isNewBreach) {
      ...
      const newToast = {
        ...
        patientName: currentGoal.patientName,
        type: status as 'low' | 'high',
        value: currentGlucose,
        limit: status === 'low' ? currentGoal.low : currentGoal.high,
        timestamp: now,
      };
  ...
  }, [currentGlucose, selectedPatientId, currentGoal.low, currentGoal.high, currentGoal.patientName]);
  ...
  return (
    <div id="dashboard-tab" className="space-y-6 max-w-6xl mx-auto relative">
      ...
      {/* Real-time Toast Notifications Floating Panel */}
        ...
        {toasts.map((toast) => (
          <div
            ...
            className={`pointer-events-auto w-full bg-[#07192d]/95 border ${
              toast.type === 'low' 
                ? 'border-[#ffb4ab] shadow-[0_10px_30px_rgba(255,180,171,0.15)]' 
                : 'border-amber-500 shadow-[0_10px_30px_rgba(245,158,11,0.12)]'
            } p-4 rounded-xl shadow-2xl backdrop-blur-md relative overflow-hidden transition-all duration-300 ease-out flex flex-col gap-2`}
          >
            ...
            <div className="flex items-center justify-between pt-1.5 border-t border-[#45464d]/20 mt-0.5 text-[10px] font-mono text-[#c6c6cd]/80">
              <span className="flex items-center gap-1">
                ...
                <span>Active Clinician Response Needed</span>
              </span>
              <button
                onClick={() => setToasts((prev) => prev.filter((t) => t.id !== toast.id))}
                className="text-[#5adace] hover:underline font-bold font-sans flex items-center gap-0.5 cursor-pointer"
              >
                Dismiss
              </button>
            </div>
